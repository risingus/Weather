...
function searchWeather(searchTerm) {
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${searchTerm}&APPID=${appID}&units=${units}&lang=${lang}`
  )
    .then((result) => {
      return result.json();
    })
...
function init(resultFromServer) {
  ...
  temp_minElement.innerHTML =
    "Min: " + Math.floor(resultFromServer.main.temp_min) + "&#176  C";
  temp_maxElement.innerHTML =
    "Max: " + Math.floor(resultFromServer.main.temp_max) + "&#176  C";
